<div align="center">
<img src="app/src/main/res/mipmap-hdpi/ic_launcher_round.webp" width="108" height="108" alt="Kick Third-Party Emotes">

<h1>Live Streaming Camera</h1>

一個使用 [StreamPack boilerplate](https://github.com/ThibaultBee/StreamPack-boilerplate) 作底層製作的中文 Android/Twitch 直播程式。
<br>[![Twitch](https://img.shields.io/badge/Twitch-9146FF?style=for-the-badge&logo=twitch&logoColor=white)](https://twitch.tv)
[![YouTube](https://img.shields.io/badge/YouTube-FF0000?style=for-the-badge&logo=youtube&logoColor=white)](https://youtube.com)
[![Android](https://img.shields.io/badge/Android-3DDC84?style=for-the-badge&logo=android&logoColor=white)]()
</div>

## 功能

- ✔️ RTMP 推流直播，Twitch 直接推流。
- ✔️ SRT 推流直播，現可選擇AAC或OPUS作音訊格式。
- ✔️ Twitch 聊天室、直播人數、直播時間。聊天室內的超連結可按下及彈出瀏覽器。
- ✔️ Youtube 聊天室。
- ✔️ 藍牙耳機收音連線。
- ✔️ 直播時顯示上傳速度，請在設定內開啟。
- ✔️ 隱藏通知欄、導航欄作全屏顯示，可在設定內開關。
- ✔️ 電池資訊、溫度顯示，可在設定內開關。
- ✔️ Wifi、Sim卡訊號顯示，可在設定內開關。
- ✔️ 左右音量顯示，可在設定內開關。
- ✔️ 手機抖動顯示，可在設定內開關。
- ✔️ Text-to-Speech聊天訊息朗讀。
- ✔️ 水平角度顯示，可在設定內開關。
- ✔️ 直播時可分別錄影影片儲存到手機記憶體內。
- ✔️ 暫時加入6種鏡頭特效：馬賽克、復古、模糊、美顏、黑白濾鏡、一開三分割畫面，不會在預覽中顯示，只有直播內見到。
- ✔️ 設定匯出/匯入功能。
- ❌ 拍攝照片。(現時有bug已隱藏，待修復)。
  
![Live-Streaming-Camera](pics/Screenshot_7.jpg)
![Live-Streaming-Camera](pics/Screenshot_2.jpg)
![Live-Streaming-Camera](pics/Screenshot_3.jpg)
![Live-Streaming-Camera](pics/Screenshot_4.jpg)
## 更多可能加入功能

- 🔜 Overlay


## 按鍵說明
![Live-Streaming-Camera](pics/Screenshot_5.jpg)

## 安裝方法
我會在 [GitHub releases](https://github.com/kongjjj/Live-Streaming-Camera/releases) 內發布最新 .apk 檔案。

可以在手機上開啟 GitHub 發行頁面，下載 .apk 檔案並安裝。 

## 我製作的其他程式
- [Chat Overlay](https://github.com/kongjjj/Chat-Overlay)：一個Android透明Youtube/Twitch聊天室overlay給直播使用。 Android Transparent Youtube/Twitch Chat viewer app for IRL streaming.  
